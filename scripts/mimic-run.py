...
if ERROR is False:
    if params["Run"]["WF_Psi"] is True or params["Run"]["WF_Vel"] is True:
        ...
        WF_dens_k = shift.cart.forward_mpi_fft_3D(WF_dens, x_shape, boxsize, ngrid, FFT)
        ...
        if params["Cosmology"]["ScaleDepGrowth"] is False or params["Run"]["WF_Psi"] is True:
            ...
            _WF_psi_x = shift.cart.backward_mpi_fft_3D(WF_psi_kx, x_shape, boxsize, ngrid, FFT)
            _WF_psi_y = shift.cart.backward_mpi_fft_3D(WF_psi_ky, x_shape, boxsize, ngrid, FFT)
            _WF_psi_z = shift.cart.backward_mpi_fft_3D(WF_psi_kz, x_shape, boxsize, ngrid, FFT)
...
if ERROR is False:
    if params["Run"]["WF_Vel"] is True:
        ...
        if params["Cosmology"]["ScaleDepGrowth"] is False:
            ...
            WF_psi_kx = shift.cart.forward_mpi_fft_3D(WF_psi_x, x_shape, boxsize, ngrid, FFT)
            WF_psi_ky = shift.cart.forward_mpi_fft_3D(WF_psi_y, x_shape, boxsize, ngrid, FFT)
            WF_psi_kz = shift.cart.forward_mpi_fft_3D(WF_psi_z, x_shape, boxsize, ngrid, FFT)
            ...
            WF_vel_x = shift.cart.backward_mpi_fft_3D(WF_vel_kx, x_shape, boxsize, ngrid, FFT)
            WF_vel_y = shift.cart.backward_mpi_fft_3D(WF_vel_ky, x_shape, boxsize, ngrid, FFT)
            WF_vel_z = shift.cart.backward_mpi_fft_3D(WF_vel_kz, x_shape, boxsize, ngrid, FFT)
...
if ERROR is False:
    if params["Run"]["CR"] is True or params["Run"]["IC"] is True:
        ...
        WN_k = shift.cart.forward_mpi_fft_3D(WN, x_shape, boxsize, ngrid, FFT)
        ...
        _dens = shift.cart.backward_mpi_fft_3D(dens_k, x_shape, boxsize, ngrid, FFT)
        ...
        _psi_x = shift.cart.backward_mpi_fft_3D(psi_kx, x_shape, boxsize, ngrid, FFT)
        _psi_y = shift.cart.backward_mpi_fft_3D(psi_ky, x_shape, boxsize, ngrid, FFT)
        _psi_z = shift.cart.backward_mpi_fft_3D(psi_kz, x_shape, boxsize, ngrid, FFT)
...
if ERROR is False:
    if params["Run"]["IC"] is True:
        ...
        if params["Cosmology"]["ScaleDepGrowth"] is False:
            ...
            dens_k = shift.cart.forward_mpi_fft_3D(dens, x_shape, boxsize, ngrid, FFT)
            ...
            dens = shift.cart.backward_mpi_fft_3D(dens_k, x_shape, boxsize, ngrid, FFT)
